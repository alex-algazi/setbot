...
### B. Problems/Issues
1. Had an issue where the queries were being not being executed synchronously, which led to data not being retrieved before it was being operated on. Fixed by using two promise functions, one for getting a single result, and one for getting all results of a query. (near the top of showstats.js)
